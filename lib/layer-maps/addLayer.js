...
function createAddLayersPanel() {
	var html = "<div class=add-layer-panel>" + "<label>Please select input type</label><br>" + "<input type='radio' name='fileType' value='shape' onChange='setDataUploadType(this.value)' checked=true> Shape File" + "<input type='radio' name='fileType' value='raster' onChange='setDataUploadType(this.value)'> Raster File<br><br>" + "<div id='shape-file-data' class='grey-border'>" + "<label>Select shape file   </label>" + "<input type='file' id='inputShpFiles' name='files[]'><br><br>" + "<label>Select dbf file   </label>" + "<input type='file' id='inputVectorDbfFiles' name='files[]'><br><br>" + "<input type='submit' class='' id='upload-submit-shape' value='Go!' onclick='displayShapeDataForm()'>" + "<br><br>" + "<div id='dbf-data-table-div' class='dbf-table'></div>" + "</div>" + "<div id='raster-file-data' class='hide grey-border'>" + "<label>Select tiff file   </label>" + "<input type='file' id='inputTiffFiles' name='files[]'><br><br>" + "<label>Select dbf file   </label>" + "<input type='file' id='inputRasterDbfFiles' name='files[]'><br><br>" + "<input type='submit' class='' id='upload-submit-raster' value='Go!' onclick='displayRasterDataForm()'>" + "<br><br>" + "</div>" + "</div>";
...
function addMetadataComponent(col_names) {
	...
	var styleColumnHtml = "";
	if (col_names) {
		styleColumnHtml = "<tr><td data-name='default_style_by'>Default Styling Column</td><td>" + "<select id='default_style_column'>";
		col_names.forEach(function (column) {
			styleColumnHtml += "<option value='" + column + "'>" + column + "</option>";
		});
		styleColumnHtml += "</select>" + "</td></tr>";
	}
	var detailsHtml = "<table id='metadata_table'>" + styleColumnHtml + "<tr><td data-name='layer_name'>Layer Name</td><td><input/></td></tr>" + "<tr><td data-name='layer_description'>Layer Description</td><td><textarea style='width:100%' rows='5' cols='50'></textarea></td></tr>";

	if (dataType === "shape") {
		detailsHtml += "<tr><td data-name='layer_type'>Layer Type</td><td>" + getLayerTypeSelector() + "</td></tr>";
	}

	detailsHtml += "<tr><td data-name='created_by'>Contributor</td><td><input/></td></tr>" + "<tr><td data-name='attribution'>Attribution</td><td><input/></td></tr>" + "<tr><td data-name='license'>License</td><td>" + getLicenseSelector() + "</td></tr>" + "<tr><td data-name='created_date'>Data Curation Date</td><td><input id='curation_date_picker'></td></tr>" + "<table>";
...
function createDataTable(source, callback) {
	...
	source.read().then(function (result) {
		...
		function addRow(result1) {
			...
			table_html += "</tr>";
			row_count += 1;
			if (!result1.done && row_count < MAX_ROW_COUNT) source.read().then(addRow);else {
...
function uploadFiles() {
	...
	var url = "http://" + _config2.default.geoserver_path + "/naksha/geoserver/uploadshp";
	console.log(url);
